...
<template>
  <TheContainer>
    ...
    <TheInfiniteLoading
      ...
      :infiniteHandler="loadCardsDataPortion"
    />
  </TheContainer>
</template>
<!-- eslint-enable -->
    ...
    methods: {
      async loadCardsDataPortion($state?: any) {
        if (this.cardDataIds.length === 0) {
          ...
          const cardDataIds = await request(`${cardDataType}stories.json`);
          this.cardDataIds = cardDataIds;
        }
        if (!this.isLoadedCardsData) {
          const cardsDataPortionProms: Promise<object>[] = [];
          const showMoreCardsCount = this.showMoreCardsCount;
          const cardDataIds = this.cardDataIds.slice(0, 11);
      ...
      cardsNumDivideByShowMoreCardsNumRound(): number {
        return Number(
          (this.cardDataIds.slice(0, 11).length / this.showMoreCardsNum)
            .toFixed(1)
            .split('.')[0],
        );
      },
